...
class DataValidation:
    ...
    def validate_number_of_columns(self, dataframe: DataFrame) -> bool:
        ...
        Output: Retuns bool value based on validation results
        On Failure: Write an exception log and tehn raises an exception
        """
        try:
            status = len(dataframe.columns) == len(self._schema_config["columns"])
            logging.info(f"Number of columns required correct: [{status}]")
            return status 
        except Exception as e:
            raise MyException(e, sys)
    ...
    def is_column_exist(self, df: DataFrame) -> bool:
        ...
        Description: This method validates the existence of all required columns
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        ...
        try:
            ...
            # Checking number of columns for train/test df
            status = self.validate_number_of_columns(dataframe=train_df)
            if not status:
                validation_error_msg += f"Columns are missing training dataframe. "
            else:
                logging.info(f"Number of required columns in training dataframe is correct")

            status = self.validate_number_of_columns(dataframe=test_df)
            if not status:
                validation_error_msg += f"Columns are missing in testing dataframe. "
            else:
                logging.info(f"Number of required columns in testing dataframe is correct")

            # Validating column existence in train/test df
            ...
            # Save validation satus and message to a JSON file
            validation_report = {
                "validation_status": validation_status,
